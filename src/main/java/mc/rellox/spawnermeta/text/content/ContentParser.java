...
public final class ContentParser {
	...
	private List<Text> text() {
		...
		while (m.find() == true) {
			s = m.start();
			if (s > e)
				list.add(new Text(text.substring(e, s), Type.text));
			e = m.end();
		...
		if (e < text.length())
			list.add(new Text(text.substring(e), Type.text));
		return list;
	}
	...
	private class ContentBuilder {
		...
		private void format(String s) {
			Format f = switch (s.substring(1).toLowerCase()) {
		...
		private Content build(Content input) {
			...
			if (is != null) {
				if (is.length == 1) {
					if (fs.isEmpty() == true)
						color = Colorer.of(is[0]);
					else
						color = Colorer.of(is[0], Format.of(fs));
				} else {
					if (fs.isEmpty() == true)
						color = Colorer.of(Colors.of(is));
					else
						color = Colorer.of(Colors.of(is), Format.of(fs));
				}
			} else if (fs.isEmpty() == false)
				color = Colorer.of(Colors.white, Format.of(fs));
			else
				color = null;
